...
updateScript() {                     ;Create Directory Structure - Update script from Github
 ...
 UrlDownloadToFile, https://raw.githubusercontent.com/TyGreenyy/CarOfferAHK/main/resources/DealerCodes.ini, %A_MyDocuments%\CarOfferAHK\resources\DealerCodes.ini
  ...
  if (version = 345){                                             ;Downloads new .ahk if version does not match
      } else {
...
VINAnalysisNEW(){
    ...
    openLink := "https://opsweb.prod.pearlsolutions.com/#/vin-analysis/" . searchTerm
    toast("Searching VIN Analysis", searchTerm, ,2000)
    shellrun(openLink)
    Clipboard := searchTerm
    return
...
getHubspotEngagement(){
	Send, ^c
	Sleep, 200
	test := Clipboard
	html := WinClip.gethtml(Clipboard)
	Clipboard := html
	RegexMatch(html, "SourceURL:(.*)", SourceURL)
		SourceURL := StrSplit(SourceURL, "SourceURL:")
		;;https://app.hubspot.com/contacts/5712725/contact/76370/
		RegexMatch(html, "data-selenium-id=\D(\d+)", TestVar)
		TestVar := StrSplit(TestVar, """")
		;;https://app.hubspot.com/contacts/5712725/contact/947258/?engagement=13708870157
		engagementURL := SourceURL[2] . "?engagement=" . TestVar[2]
		ClipBoard := engagementURL
		if (TestVar[2])
			toast("Success! Copied to Clipboard:", engagementURL, 12 ,3000, true)
		Else
			toast("No Match Found", "Highlight Single Hubspot Activity", ,5000)
			return
}
...
handsellCopy(){
            ...
            {
            LOC++
            if (RegExMatch(A_Loopfield, "^20\d{2,2}", year)){
                ymmLOC := LOC
                rp%ymmLOC% := StrReplace(StrReplace(A_Loopfield, "`n", ""), "`r", "")
               }
            if (RegExMatch(A_Loopfield, "^\$\d+\,\d+", price)){
                priceLOC := LOC
                price := StrReplace(StrReplace(StrReplace(StrReplace(A_Loopfield, "`n", ""), "`r", ""), ",", ""), "$", "")
                price := (price + 1500)
                price%priceLOC% := "$" SubStr(price, -(StrLen(price)), (StrLen(price)-3)) "," SubStr(price, -2, 3)
                 }
          if (RegExMatch(A_Loopfield, "VIN:", VIN)){
            VINLOC := LOC
            VIN%VINLOC% := StrReplace(StrReplace(StrReplace(A_Loopfield, "`n", ""), "VIN:", ""), "`r", "")
              }
           if (RegExMatch(A_Loopfield, "Miles:", miles)){
                milesLOC := LOC
                miles%milesLOC% := StrReplace(StrReplace(A_Loopfield, "`n", ""), "`r", "")
                linenum++
                line%linenum% := rp%ymmLOC% " - " VIN%VINLOC% " - " miles%milesLOC% " - " price%priceLOC%
               }
           }
            Clipboard := html
             ...
             Loop, Parse, NewHTML, @
              {
                   ...
                   linkLOC := A_Index-1
                   hreflink%linkLOC% := hreflink.Value(1)
                }
               }
            Loop, %linenum%
            {
                If (A_Index = 1)
                     comblines := line%A_Index% "`n" hreflink%A_Index% 
                else
                    comblines := comblines "`n`n" line%A_Index% "`n" hreflink%A_Index% 
            }
             toast("Success! Copied to Clipboard:", comblines, 12 ,3000, true)
            Clipboard := comblines
            return
    }
...
yearmakemodelformat2(){
    searchTerm := Clip()
      LOC := 0, linenum := 0
      Loop, Parse, searchTerm, `n
        {
        LOC++
      if (RegExMatch(A_Loopfield, "^20\d{2,2}", year)){
        ymmLOC := LOC
        rp%ymmLOC% := StrReplace(caseChange(StrReplace(A_Loopfield, "`n", ""), "T"), "`r", "")
         }
         if (RegExMatch(A_Loopfield, "VIN", VIN)){
            vinline := (A_Index+1)
         }
        ...
        if (RegExMatch(A_Loopfield, "Sell Today", price)){
            priceline := (A_Index+1)
            }
        if (A_index = priceline){
            priceLOC := A_Index
            price%priceLOC% := A_Loopfield
            linenum++
            line%linenum% := rp%ymmLOC% " - " VIN%VINLOC% " - Sell Today: " price%priceLOC%
            }
        }
        Loop, %linenum%
        {
            If (A_Index = 1)
             comblines := line%A_Index%
            else
             comblines := comblines "`n" line%A_Index%
        }
        ;comblines := st_columnize(comblines, "-", 1, " ", A_Tab) 
        if !(comblines = ""){
            Clipboard := comblines
            toast("Success! Copied to Clipboard:", comblines, 12 ,3000, true)
        } else {
            toast("Tip:","`nHighlight Right to Left`nfrom Sell Today to the Year Make Model", 12, 5000, true)
        }
    }
...
dealerstats(){
    ...
    toast(dealershipName "`nDealer Stats Summary", "", ,2000)
    ShellRun(openLink)
    return
}
...
dealerExclu(){
    ...
    toast(dealershipName "`nDealer Exclusions", "", ,2000)
    ShellRun(openLink)
    return
}
...
matrixOverview(){
    ...
    toast(dealershipName "`nMatrix Overview", "", ,2000)
    ShellRun(openLink)
    return
}
...
matrixChanges(){
    ...
    toast(dealershipName "`nMatrix Changes", "", ,2000)
    ShellRun(openLink)
    return
}
...
dealerCDS(){
    ...
    toast(dealershipName "`nCDS Report", "", ,2000)
    ShellRun(openLink)
    return
}
...
groupCDS(){
    ...
    toast(groupName "`nCDS Report", "", ,2000)
    ShellRun(openLink)
    return
}
...
groupWholeSale(){
    ...
    if !(GroupID = "") {
        openLink := "http://ops.pearlsolutions.com/rdPage.aspx?rdReport=Caroffer.DealerInventoryOffers&p_groupid="  . GroupID . "&p_reportType=&p_wholesale=W&LinkHref=True"
            toast(dealershipName "`nWholesale", groupName, ,5000)
            Shellrun(openLink)
    } else if !(dealershipID = "") {
        openLink := "http://ops.pearlsolutions.com/rdPage.aspx?rdReport=Caroffer.DealerInventoryOffers&p_dealershipid="  . dealershipID . "&p_reportType=&p_wholesale=W&LinkHref=True"
            toast(dealershipName "`nWholesale", "", ,2000)
            Shellrun(openLink)
    } else {
        toast("No Match Found", "`nHighlight a Dealer Name", ,5000)
    }
...
auctionCaps(){
    ...
    toast(dealershipName "`nAuction Caps", "", ,2000)
...
dealerAccepts(){
    ...
    toast(dealershipName "`nAccepted Offers", "", ,2000)
    ShellRun(openLink)
    return
}
...
dealerOG(){
    ...
    toast(dealershipName "`nOfferGuards", "", ,2000)
    ShellRun(openLink)
    return
}
...
dealerPuts(){
    ...
    toast(dealershipName "`nPut Bids", "", ,2000)
    ShellRun(openLink)
    return
}
