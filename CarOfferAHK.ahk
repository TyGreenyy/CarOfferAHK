...
updateScript() {                     ;Create Directory Structure - Update script from Github
 ...
 UrlDownloadToFile, https://raw.githubusercontent.com/TyGreenyy/CarOfferAHK/main/resources/DealerCodes.ini, %A_MyDocuments%\CarOfferAHK\resources\DealerCodes.ini
  ...
  if (version = 222){                                             ;Downloads new .ahk if version does not match
      } else {
...
VINAnalysis(){
	...
	toast("Searching VIN Analysis", searchTerm, ,2000, false)
	shellrun(openLink)
...
matrixOverview(){
	...
	openlink := "http://ops.pearlsolutions.com/rdPage.aspx?rdReport=Caroffer.MatrixOverview&p_dealershipID=" . dealershipID . "&rdAjaxCommand=RefreshElement&LinkHref=True&rdRequestForwarding=Form"
...
toast(t, msg:="", s:=14, l:=2000, n:="") {
		...
		this.id:=toastCount
		if (n = false){
			msg := st_linewrap(msg, 60, " ")
		}
		msg := RegExReplace(msg, "`am)^[\s\R]*")
...
example: st_lineWrap("aaabbbcccdddeeefffggghhhiiijjjkkklll", 20, "---")
output:
